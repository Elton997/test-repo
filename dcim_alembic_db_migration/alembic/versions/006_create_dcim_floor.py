"""
create dcim_floor table

Revision ID: 006_create_dcim_floor
Revises: 005_create_dcim_wing
...
revision = "006_create_dcim_floor"
down_revision = "005_create_dcim_wing"
branch_labels = None
depends_on = None

TABLE_NAME = "dcim_floor"
SCHEMA = "dcim"
...
def _create_table() -> None:
    ...
    if not index_exists(SCHEMA, "ix_dcim_floor_location_id"):
        op.create_index("ix_dcim_floor_location_id", TABLE_NAME, ["location_id"], schema=SCHEMA)
    if not index_exists(SCHEMA, "ix_dcim_floor_building_id"):
        op.create_index("ix_dcim_floor_building_id", TABLE_NAME, ["building_id"], schema=SCHEMA)
    if not index_exists(SCHEMA, "ix_dcim_floor_wing_id"):
        op.create_index("ix_dcim_floor_wing_id", TABLE_NAME, ["wing_id"], schema=SCHEMA)
...
def downgrade() -> None:
    ...
    op.drop_index("ix_dcim_floor_wing_id", table_name=TABLE_NAME, schema=SCHEMA)
    op.drop_index("ix_dcim_floor_building_id", table_name=TABLE_NAME, schema=SCHEMA)
    op.drop_index("ix_dcim_floor_location_id", table_name=TABLE_NAME, schema=SCHEMA)
